...
class MenuApp {
    ...
    setupEventListeners() {
        ...
        document.getElementById('import-recipes-btn').addEventListener('click', () => {
            if (confirm('确定要导入菜谱数据库吗？这将清空现有菜品和分类，并导入新的菜谱数据。')) {
                this.initFamilyRecipes();
            }
        });
        ...
        // 菜品库导入菜谱数据库按钮事件已在setupEventListeners中设置
        ...
        if (addDishBtn) {
            ...
            // 如果按钮还没有被渲染，等待DOM更新后再添加事件监听
            setTimeout(() => {
                const delayedAddDishBtn = document.getElementById('add-dish-btn');
                if (delayedAddDishBtn) {
                    delayedAddDishBtn.addEventListener('click', () => {
                        this.openDishModal();
                    });
                }
            }, 100);
    ...
    renderDishLibrary(category = 'all', searchTerm = '', sortBy = 'name') {
            ...
            categoryHtml += `
                <div class="category-tag-container">
                    ...
                    <div class="category-actions">
                        <button class="edit-category-btn" data-category="${cat}">编辑</button>
                        <button class="delete-category-btn" data-category="${cat}">删除</button>
                    </div>
                </div>
            `;
        });
        ...
        if (filteredDishes.length === 0) {
            dishGrid.innerHTML = '<p style="text-align: center; color: #999; font-size: 18px; padding: 50px;">暂无菜品，点击"导入菜谱数据库"添加</p>';
            return;
        }
        ...
        filteredDishes.forEach(dish => {
            ...
            dishCard.innerHTML = `
                ...
                <div class="dish-actions">
                    <button class="dish-action-btn edit-btn" data-dish-id="${dish.id}">编辑</button>
                    <button class="dish-action-btn delete-btn" data-dish-id="${dish.id}">删除</button>
                </div>
            `;
    ...
    initFamilyRecipes() {
        ...
        alert('家庭菜谱数据库已生成，包含117道适合家庭制作的菜品！');
    }
}
